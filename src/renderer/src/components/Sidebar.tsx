...
const SidebarItem: React.FC<{
  ...
  >
    <div
      className={`transition-colors duration-200 ${isActive ? 'text-neon-blue' : color || 'text-current'}`}
    >
      {icon}
    </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-thin">
        {/* Section: Downloads */}
        <div
          className={`px-4 mb-2 text-[10px] font-bold text-text-dim uppercase tracking-wider transition-opacity duration-200 ${isCollapsed ? 'text-center opacity-50' : 'opacity-100'}`}
        >
      ...
      {!isCollapsed && (
        <div className="p-4 border-t border-white/5 text-center">
          <div className="text-[10px] text-text-dim/50 font-mono">v1.3.0</div>
        </div>
      )}
    </div>
  )
