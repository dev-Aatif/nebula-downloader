...
async function runDoSAttack() {
  ...
  for (let i = 0; i < 50; i++) {
    requests.push(
      request('GET', `/api/metadata?url=https://www.youtube.com/watch?v=dQw4w9WgXcQ&i=${i}`)
    )
  }
...
async function runPayloadPoisoning() {
  ...
  const resA = await request('POST', '/api/download', {
    url: 'http://example.com',
    padding: largeString
  })
  log(
    resA.statusCode === 413 ? colors.green : colors.red,
    `Large Payload Result: ${resA.statusCode} (Expected 413)`
  )

  // Case B: Malformed JSON
  log(colors.cyan, 'Test B: Malformed JSON')
  const resB = await request('POST', '/api/download', "{ 'url': 'bad json' ", {
    'Content-Type': 'application/json'
  })
  log(
    resB.statusCode === 400 ? colors.green : colors.red,
    `Malformed JSON Result: ${resB.statusCode} (Expected 400)`
  )

  // Case C: Type Injection (Format ID as object)
  log(colors.cyan, 'Test C: Type Injection')
  const resC = await request('POST', '/api/download', {
    url: 'http://example.com/video',
    formatId: { $gt: '' }
  })
  log(
    resC.statusCode === 400 ? colors.green : colors.red,
    `Type Injection Result: ${resC.statusCode} (Expected 400)`
  )
}
...
async function runConcurrencyTest() {
  ...
  for (let i = 0; i < 10; i++) {
    requests.push(
      request('POST', '/api/download', { url: `https://www.youtube.com/watch?v=testVideo${i}` })
    )
  }
  ...
  log(
    uniqueIds.size === ids.length ? colors.green : colors.red,
    `Unique IDs: ${uniqueIds.size}/${ids.length}`
  )
