...
beforeEach(() => {
    mockApi = {
        ...
        getDependencyStatus: vi.fn(() => Promise.resolve({ ytDlp: { installed: true, version: '1.0' }, ffmpeg: { installed: true, version: '1.0' } })),
        installYtDlp: vi.fn(),
        installFfmpeg: vi.fn(),
...
describe('Settings Component (settings.tsx)', () => {
    ...
    it('allows changing the output directory', async () => {
        ...
        await waitFor(() => {
            expect(mockApi.updateSettings).toHaveBeenCalledWith(expect.objectContaining({ downloadDirectory: '/new/fake/dir' }))
        }, { timeout: 2000 })
    })
    ...
    it('allows changing auto-download setting', async () => {
        ...
        await waitFor(() => {
            expect(mockApi.updateSettings).toHaveBeenCalledWith(expect.objectContaining({ autoDownload: true }))
        }, { timeout: 2000 })
    })
})
